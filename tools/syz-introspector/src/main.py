...
def extract_source_loc_analysis(workdir: str, report) -> None:
    ...
    all_c_files = deepfuzz_utils.get_c_files_mentioned_in_light(
        workdir)
...
def analyse_ioctl_handler(ioctl_handler, workdir, target_path):
    ...
    calltree = deepfuzz_utils.extract_calltree_light(
    ...
    if calltree:
        ...
        try:
            syz_core.create_deepfuzz_html_report(
...
def extract_ioctls_in_driver(kernel_folder, report, workdir, strict_ioctls):
    ...
    c_files_in_driver = deepfuzz_utils.get_c_files_mentioned_in_light(
        workdir)
    if strict_ioctls:
        refined_ioctls = []
