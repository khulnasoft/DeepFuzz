...
import yaml
from deepfuzz import commands

import textual_source_analysis
import deepfuzz_utils
...
def extract_header_files_referenced(workdir) -> Set[str]:
    ...
    raw_header_file_references = deepfuzz_utils.get_all_header_files_in_light(
        workdir)
...
def get_ioctl_handlers(ioctls, kernel_folder, fi_data_dir):
    ...
    all_functions = deepfuzz_utils.get_light_functions(fi_data_dir)

    source_files_to_functions_mapping = deepfuzz_utils.get_source_to_functions_mapping(
        all_functions)
...
def create_deepfuzz_html_report(workdir, target, entry_point_func,
                                         html_idx) -> None:
