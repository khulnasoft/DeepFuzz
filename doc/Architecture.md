...
The overall workflow of deepfuzz can be visualised as follows:
![Functions table](/doc/img/deepfuzz-architecture.png)

The two main parts to deepfuzz is the **compilation-based static analysis**
and **post-processing**. Deepfuzz has capabilities to integrate data from
runtime coverage collection, and in order to use this features deepfuzz
relies on external infrastructure to extract this coverage. Focus is particularly
on using OSS-Fuzz for this, although deepfuzz can be integrated into
other workflows.
...
analysis. As such, in order to use deepfuzz your code must be able
...
To use the deepfuzz pass simply compile a given project and the fuzzers
...
for using deepfuzz, see [Custom clang](#custom-clang).

### Custom clang

Deepfuzz relies on a custom clang in order to run. The reason for this
...
[here](https://github.com/khulnasoft/deepfuzz/issues/57) for discussions on this.
...
The code for this is located in [src/deepfuzz](/src/deepfuzz)
...
The post-processing part has some core analysis in the [src/deepfuzz](/src/deepfuzz) and
also plugin-like analyses in [src/deepfuzz/analyses](/src/deepfuzz/analyses).
...
to improve the fuzzing, e.g. the analysis plugin [engine_input.py](/src/deepfuzz/analyses/engine_input.py)
