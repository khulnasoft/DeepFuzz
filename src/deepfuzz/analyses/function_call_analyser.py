...
from deepfuzz import cfg_load
from deepfuzz import html_helpers
from deepfuzz import utils
...
class ThirdPartyAPICoverageAnalyser(analysis.AnalysisInterface):
    ...
    def third_party_func_profile(
        ...
        target_list = [
            fd for fd in profile.all_functions.values()
            if not fd.function_source_file
        ]
    ...
    def analysis_func(self,
        ...
        for fd in func_profile_list:
            if proj_profile.target_lang == "rust":
                ...
                if func_name:
                    func_name = utils.locate_rust_fuzz_item(
                        func_name, functions_of_interest)
            else:
            ...
            for called_location in called_location_list:
                ...
                hit = "Yes" if (called_location
                                in reachable_func_list) else "No"
