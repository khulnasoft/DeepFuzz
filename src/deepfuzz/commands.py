...
from deepfuzz import diff_report
from deepfuzz import html_report
from deepfuzz import utils

from deepfuzz.exceptions import DataLoaderError
from deepfuzz.frontends import oss_fuzz
...
def correlate_binaries_to_logs(binaries_dir: str) -> int:
    pairings = utils.scan_executables_for_deepfuzz_logs(binaries_dir)
    logger.info("Pairings: %s", str(pairings))
    with open("exe_to_deepfuzz_logs.yaml", "w+") as etf:
